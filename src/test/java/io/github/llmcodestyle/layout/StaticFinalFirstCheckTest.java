...
class StaticFinalFirstCheckTest {
    ...
    void violationMessageContainsFieldName() throws Exception {
        ...
        assertTrue(messages.stream().anyMatch(m -> m.contains("CONSTANT_AFTER_INSTANCE")),
            "Should mention field name in message: " + messages);
        assertTrue(messages.stream().anyMatch(m -> m.contains("ANOTHER_CONST")),
            "Should mention field name in message: " + messages);
    }

    @Test
    void staticNonFinalFieldDoesNotTriggerBarrier() throws Exception {
        List<AuditEvent> violations = runTreeWalkerCheck(StaticFinalFirstCheck.class,
            "layout/invalid/StaticFinalFirstEdgeCases.java", NO_PROPS);
    ...
    void constructorActsAsBarrier() throws Exception {
        List<AuditEvent> violations = runTreeWalkerCheck(StaticFinalFirstCheck.class,
            "layout/invalid/StaticFinalFirstEdgeCases.java", NO_PROPS);
        assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("AFTER_CTOR")),
            "static final after ctor should be flagged: " + format(violations));
    }
}
