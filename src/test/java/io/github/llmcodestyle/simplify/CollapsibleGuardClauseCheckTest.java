...
class CollapsibleGuardClauseCheckTest {
    ...
    void violationMessageMentionsGuard() throws Exception {
        for (AuditEvent event : run("simplify/invalid/CollapsibleGuardClauseInvalid.java")) {
    ...
    void bareReturnGuardDetected() throws Exception {
        ...
        assertEquals(1, violations.size(), "Bare return guard should be detected: " + format(violations));
    }
    ...
    void extractIfBodyHandlesThrowBody() throws Exception {
        ...
        assertTrue(violations.isEmpty(), "Guard with throw should not be collapsible: " + format(violations));
    }
