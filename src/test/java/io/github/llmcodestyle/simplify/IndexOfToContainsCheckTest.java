...
class IndexOfToContainsCheckTest {
    ...
    void charLiteralAndMethodCallArgNotFlagged() throws Exception {
        ...
        List<AuditEvent> violations = runTreeWalkerCheck(IndexOfToContainsCheck.class,
            "simplify/valid/IndexOfToContainsValid.java", Map.of());
        assertTrue(violations.isEmpty(), "char literal / method call indexOf args should be valid");
    }

    @Test
    void violationsAreExactCount() throws Exception {
        List<AuditEvent> violations = runTreeWalkerCheck(IndexOfToContainsCheck.class,
            "simplify/invalid/IndexOfToContainsInvalid.java", Map.of());
